...
class MazeEnvironment:
    ...
    """
    def __init__(self, agent, walls, start_point, exit_point, exit_range=5.0):
        """
...
def read_environment(file_path, m):
    """
    ...
    with open(file_path, 'r') as file:
        for line in file.readlines():
            ...
            if index == 0:
                ...
                line = str(np.random.randint(0, 180))
    ...
    maze_env = MazeEnvironment(agent=maze_agent, walls=walls, start_point = loc, exit_point=maze_exit)
    while maze_env.test_wall_collision(loc):
        random.seed(1)
        if m == "medium":
            x = np.random.randint(0, 280)
            y = np.random.randint(0, 120)
    ...
    # maze_env = MazeEnvironment(agent=maze_agent, walls=walls, exit_point=maze_exit)
    print("Random starting location: "+str(loc))
...
def maze_simulation_evaluate1(env, net, time_steps, mcns=0.0, n_item=None, path_points=None):
    """
    The function to evaluate maze simulation for specific environment
    and controll ANN provided. The results will be saved into provided
    agent record holder.
