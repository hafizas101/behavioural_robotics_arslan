...
def eval_fitness(genome_id, genome, config, time_steps=700):
    """
...
def run_experiment(config_file, maze_env, trial_out_dir, args=None, n_generations=100, silent=False):
    ...
    p.add_reporter(neat.Checkpointer(50, filename_prefix='%s/maze-neat-checkpoint-' % trial_out_dir))
...
#    print("**************** Generation Wise results *******************")
#    fitness_generations = stats.get_fitness_mean()
#    print("Fitness: "+str(fitness_generations)+", Length: "+str(len(fitness_generations)))
#    species = stats.get_species_sizes()
#    print("Species Count: "+str(species)+", Length: "+str(len(species)))
    ...
    if not silent or solution_found:
        ...
        if args is None:
            ...
            visualize.draw_maze_records(maze_env, trialSim.record_store.records, 
                                        ...
                                        width=args.width,
                                        height=args.height,
                                        filename=os.path.join(trial_out_dir, 'maze_records.png'))
        visualize.plot_stats(stats, ylog=False, view=True, filename=os.path.join(trial_out_dir, 'avg_fitness.png'))
        visualize.plot_species(stats, view=True, filename=os.path.join(trial_out_dir, 'speciation.png'))
        ...
        path_points = []
        evaluate_fitness = maze.maze_simulation_evaluate1(
                                    env=maze_env, 
                                    net=control_net, 
                                    time_steps=700,
                                    path_points=path_points)
        print("Evaluated fitness of best agent: %f" % evaluate_fitness)
        visualize.draw_agent_path(maze_env, path_points, best_genome,
                                    view=show_results, 
                                    width=args.width,
                                    height=args.height,
                                    filename=os.path.join(trial_out_dir, 'best_solver_path.png'))
